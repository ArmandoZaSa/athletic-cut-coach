// Athletic Cut Coach v4.8.14 — recovery state synchronization
(function(){
'use strict';
const VERSION='v4.8.14';
function bridge(){try{if(!window.db&&typeof db!=='undefined')window.db=db;if(!window.db)window.db={health:{daily:{}}};if(!window.db.health)window.db.health={daily:{}}}catch(e){}}
const nz=v=>typeof v==='number'&&Number.isFinite(v)&&v>0;
const D=()=>{bridge();return window.db?.health?.daily||{}};
...
function apply(){try{bridge();stamp();sync()}catch(e){console.error('v4.8.14 recovery sync',e)}}
function boot(){apply();const old=window.render;if(typeof old==='function'&&!old.__v4814){const w=function(){bridge();const r=old.apply(this,arguments);queueMicrotask(apply);return r};w.__v4814=true;window.render=w}window.addEventListener('cutcoach-health-hydrated',()=>queueMicrotask(apply));setTimeout(apply,500);setTimeout(apply,1500)}
