...
(function(){
  ...
  function latestMetric(key){for(const d of days()){const v=db.health.daily[d]?.[key];if(v!==null&&v!==undefined&&v!==0)return{v,date:d,source:db.health?.provenance?.[d]?.[key]?.source||null}}return null}
  ...
  function shortType(t){return String(t||'').replace(/^HK(?:Quantity|Category|Characteristic|Correlation)TypeIdentifier/,'').replace(/^HKDataTypeIdentifier/,'')}
  window.renderHealthKitDiagnostics=function(){
    const salud=document.getElementById('salud');if(!salud)return;
  ...
  window.renderSourceInsights=function(){
    ...
    const stats=db?.health?.sourceStats||{},names=Object.keys(stats);if(!names.length){box.innerHTML='<div class="small">Importa export.xml para ver el desglose y procedencia por fuente.</div>';return}
    const relevant=names.filter(n=>/huawei|gravl|health|iphone|watch/i.test(n)).sort((a,b)=>{const aa=/huawei/i.test(a)?0:/gravl/i.test(a)?1:2,bb=/huawei/i.test(b)?0:/gravl/i.test(b)?1:2;return aa-bb}),use=(relevant.length?relevant:names).slice(0,10);
